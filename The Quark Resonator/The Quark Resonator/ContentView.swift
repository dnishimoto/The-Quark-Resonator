...
//
//  Added:
//  - Electrical input-energy integration
...
final class QuarkResonatorEngine: ObservableObject {
    ...
    private var configuration = ResonatorConfiguration()
    ...
    func updateConfiguration(
        _ newConfiguration: ResonatorConfiguration
    ) {

        configuration = newConfiguration

        state.effectiveMassKg =
            newConfiguration.effectiveMassKg

        state.restoringConstant =
            newConfiguration.restoringConstant

        state.damping =
            newConfiguration.damping

        state.targetFrequencyHz =
            QRConstants.targetFrequencyHz

        state.qrtlCoupling =
            newConfiguration.qrtlCoupling

        state.inputVoltageV =
            newConfiguration.inputVoltage
    ...
    func calculateNaturalFrequency() {
        ...
        let mass =
            max(configuration.effectiveMassKg, 1.0e-60)

        let stiffness =
            max(configuration.restoringConstant, 0.0)
    ...
    private func updateFrequencyError() {
        ...
        state.frequencyErrorPercent =
            abs(state.frequencyErrorHz) /
            max(state.targetFrequencyHz, 1.0) *
            100.0
    ...
    func calculateResonanceResponse(
        frequencyHz: Double
    ) -> Double {

        let f0 =
            max(state.naturalFrequencyHz, 1.0)
        ...
        let stiffness =
            max(state.restoringConstant, 1.0e-60)

        let damping =
            max(state.damping, 0.0)
    ...
    private func selectIdentifiedResonantMode() {
        ...
        // First prefer modes that actually satisfy
        // the resonance-response threshold.
        ...
        if !resonantModes.isEmpty {
            ...
            // If no mode crosses the resonance
            // threshold, retain the strongest
            // candidate for diagnostic purposes.
        ...
        if state.targetModeDetected {
            ...
            state.statusMessage =
                "TARGET MODE DETECTED"

        } else if state.resonantModeDetected {

            state.statusMessage =
                "RESONANT MODE DETECTED"
        }
    }
    ...
    func calculateLoss() {
        ...
        if state.qualityFactor.isFinite &&
            state.qualityFactor > 0.0 {
    ...
    func increasePowerIfInsufficient(
        deltaTime: Double
    ) {
    ...
    func step(
        ...
        // Integrate actual electrical input energy.
        ...
        // use the identified resonant mode.
        ...
        // Reevaluate modes after resonator update.
    ...
    func calculateHydrogenResponse() {
        ...
        // Hydrogen responds only to generated
        // resonator energy, not simply to the
...
@MainActor
final class QuarkResonatorSceneController:
    ObservableObject {

    let scene = SCNScene()

    private(set) var resonatorNode =
        SCNNode()

    private(set) var fieldNode =
        SCNNode()

    private(set) var hydrogenNode =
        SCNNode()

    private(set) var energyNode =
        SCNNode()

    private var hydrogenRotationAction =
        false
    ...
    private func buildResonator() {
        ...
        outer.firstMaterial?.diffuse.contents =
            UIColor.cyan

        outer.firstMaterial?.emission.contents =
            UIColor.cyan
        ...
        inner.firstMaterial?.diffuse.contents =
            UIColor.systemBlue

        inner.firstMaterial?.emission.contents =
            UIColor.systemBlue
    ...
    private func buildHydrogen() {
        ...
        nucleus.firstMaterial =
            SCNMaterial()

        nucleus.firstMaterial?.diffuse.contents =
            UIColor.red

        nucleus.firstMaterial?.emission.contents =
            UIColor.red
        ...
        for radius in [
            ...
            shell.firstMaterial?.diffuse.contents =
                UIColor.yellow

            shell.firstMaterial?.emission.contents =
                UIColor.yellow
    ...
    private func buildEnergyField() {
        ...
        sphere.firstMaterial?.transparency =
            0.12
    ...
    func update(
        ...
        // Do not create a new infinite
        // SCNAction on every UI update.

        if state.running &&
            !hydrogenRotationAction {
...
struct QuarkResonatorSceneView:
    ...
    func makeUIView(
        ...
        if let camera =
            controller.scene.rootNode.childNode(
                withName: "MainCamera",
                recursively: true
            ) {
...
struct ContentView: View {
    ...
    @StateObject
    private var engine =
        QuarkResonatorEngine()

    @StateObject
    private var sceneController =
        QuarkResonatorSceneController()
    ...
    private var estimatedShellEnergyEV: Double {
        let joules = abs(engine.state.hydrogenEnergyChangeJ)

        return joules / 1.602_176_634e-19
    }
    ...
    private var hydrogenFusionStatus: (
        ...
        /*
         This is a UI/model threshold, not a physical prediction.

         10 keV = 10,000 eV. It is a deliberately conservative
         "fusion-relevant energy regime" marker for this simulation.
        */
    ...
    private var frequencyKey: some View {
        ...
        VStack(
            ...
            Group {
                ...
                Text(
                    String(
                        format: "Shell Energy: %.3e eV",
                        estimatedShellEnergyEV
                    )
                )
                .font(
                    Font.system(
            ...
            Text(
                "TARGET \(formatFrequency(QRConstants.targetFrequencyHz))"
            )
            .font(
                Font.system(
                    size: 22,
            ...
            Text(
                "ACTUAL \(formatFrequency(engine.state.outputFrequencyHz))"
            )
            .font(
                Font.system(
                    size: 15,
            ...
            .foregroundStyle(
                engine.state.targetModeDetected
                ? .green
                : .orange
            )
        }
    ...
    private var frequencyPanel: some View {
        ...
        panel(
            ...
            valueRow(
                "Natural Frequency",
                formatFrequency(
                    engine.state.naturalFrequencyHz
                )
            )
            ...
            valueRow(
                "Output Frequency",
                formatFrequency(
                    engine.state.outputFrequencyHz
                )
            )
            ...
            valueRow(
                "Target Frequency",
                formatFrequency(
                    engine.state.targetFrequencyHz
                )
            )
            ...
            valueRow(
                ...
                String(
                    format:
                        "%.6f%%",
                    engine.state.frequencyErrorPercent
                )
            )
    ...
    private var electricalPanel: some View {
        ...
        panel(
            ...
            valueRow(
                "Required Power",
                formatPower(
                    engine.state.requiredInputPowerW
                )
            )
            ...
            valueRow(
                "Power Deficit",
                formatPower(
                    engine.state.powerDeficitW
                )
            )

            valueRow(
                "Feedback",
                engine.state.powerFeedbackActive
                ? "INCREASING"
                : "HOLD"
            )
        }
    ...
    private var resonancePanel: some View {
        ...
        panel(
            ...
            valueRow(
                "Mode Frequency",
                formatFrequency(
                    engine.state.resonantModeFrequencyHz
                )
            )
            ...
            valueRow(
                ...
                String(
                    format:
                        "%.6f",
                    engine.state.resonanceResponse
                )
            )
            ...
            valueRow(
                "Mode Amplitude",
                formatScientific(
                    engine.state.targetModeAmplitude
                )
            )
    ...
    private var energyPanel: some View {
        ...
        panel(
            ...
            valueRow(
                "Target Mode Energy",
                formatEnergy(
                    engine.state.targetModeEnergyJ
                )
            )
            ...
            valueRow(
                ...
                formatEnergy(
                    engine.state.generatedModeEnergyJ
                )
            )
        ...
        panel(
            ...
            Toggle(
                ...
                isOn:
                    Binding(
                        get: {
                            engine.state.qrtlEnabled
                        },
                        set: {
                            engine.setQRTLEnabled(
                                $0
                            )
                        }
                    )
            ...
            valueRow(
                "Shell Energy",
                formatEnergy(
                    engine.state.hydrogenShellEnergyJ
                )
            )
            ...
            valueRow(
                "Energy Change",
                formatEnergy(
                    engine.state.hydrogenEnergyChangeJ
                )
            )
        }
    }
    ...
    private var sweepPanel: some View {
        ...
        panel(
            ...
            valueRow(
                "Search Active",
                engine.state.frequencySearchActive
                ? "TRUE"
                : "FALSE"
            )
            ...
            valueRow(
                "Search Complete",
                engine.state.frequencySearchCompleted
                ? "TRUE"
                : "FALSE"
            )
            ...
            valueRow(
                "Best Frequency",
                formatFrequency(
                    engine.state.bestResponseFrequencyHz
                )
            )
        ...
        panel(
            ...
            valueRow(
                ...
                String(
                    format:
                        "%.6f rad",
                    engine.state.phaseError
                )
            )

            valueRow(
                "Phase Locked",
                engine.state.phaseLocked
                ? "TRUE"
                : "FALSE"
            )
            ...
            valueRow(
                "Energy Target",
                engine.state.energyTargetReached
                ? "REACHED"
                : "NOT REACHED"
            )
        }
